...
Vec3 resolveCircle2(Circle &a, Circle &b, float deltaTime) {
    ...
    if (dot < 0) {
        ...
        // v' = v - 2(v·n)n
        a.vel.point.x -= 2.0f * dot * unit_x;
        a.vel.point.y -= 2.0f * dot * unit_y;
    ...
    //const float DAMPING = 0.8f;  // 0.8 = 80% of original speed after bounce
...
{   
    ...
    float distance_y = a.point.y - b.point.y;  
    float r = a.radius + b.radius;
    if ((distance_x * distance_x) + (distance_y * distance_y) <= r * r){
        return true;
    } 
        return false;
}*/
